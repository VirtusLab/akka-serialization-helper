 ...
 *
 * @param oldParentChildFQCNPairs - collection that holds String pairs representing the content
 *   of the `directClassDescendantsCacheFile`. Each String pair is wrapped inside
 *   `ParentChildFQCNPair` instance. `oldParentChildFQCNPairs` variable is needed
 *   to catch missing codec registrations when using sbt incremental compilation (so - these are FQCNs that were found
 *   during previous compilation). Without using `oldParentChildFQCNPairs` we wouldn't be able
