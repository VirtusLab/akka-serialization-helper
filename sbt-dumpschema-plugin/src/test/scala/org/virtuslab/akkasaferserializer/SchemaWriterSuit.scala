...
import org.virtuslab.akkasaferserializer.model.{Codecs, Field, TypeDefinition}
import org.virtuslab.akkasaferserializer.model.TypeDefinition.ClassDefinition

class SchemaWriterSuit extends AnyWordSpecLike with should.Matchers with Codecs {
  val testDef: TypeDefinition = ClassDefinition("test", Seq("one", "two"), Seq(Field("a", "Int", Seq())))
